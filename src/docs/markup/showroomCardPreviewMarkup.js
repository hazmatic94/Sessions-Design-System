...
export function showroomCardPreviewMarkup({
  ...
  const imageMarkup = imageSrc
    ? `<img src="${escapeHtml(imageSrc)}" alt="${escapeHtml(imageAlt || title)}" loading="lazy" />`
    : '';

  return `
    <article class="joker-showroom-card">
      <div class="joker-showroom-card-top-rail">
        <div class="joker-showroom-card-brand">
          <svg class="joker-showroom-card-brand-icon" viewBox="0 0 20 20" aria-hidden="true" focusable="false" fill="none">
          ...
          <span>Joker Originals</span>
        </div>
        <button class="joker-showroom-card-icon is-favourite" type="button" aria-label="Favourite ${safeTitle}" aria-pressed="false" data-showroom-card-action>
          ${lucideIcon('heart')}
        </button>
      </div>
      <div class="joker-showroom-card-media">
        <div class="joker-showroom-card-image" data-showroom-card-image aria-hidden="true">${imageMarkup}</div>
        <a class="joker-showroom-card-media-link" href="${safeHref}" aria-label="Open ${safeTitle}"></a>
      </div>
      <div class="joker-showroom-card-content">
        <div class="joker-showroom-card-copy">
          <h3>${safeTitle}</h3>
          <p>${safeBody}</p>
        </div>
      </div>
