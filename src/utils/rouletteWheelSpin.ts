...
} from "../components/RouletteWheel/rouletteWheelLayout.js";
import {
  ROULETTE_BALL_TRACK_RADIUS,
  ROULETTE_OUTER_RADIUS,
  ROULETTE_WHEEL_CENTER,
} from "../components/RouletteWheel/rouletteWheelPaths.js";
...
const BALL_SPIRAL_END_RADIUS =
  Math.hypot(
    ROULETTE_FIXED_LANDING_POSITION.x - ROULETTE_WHEEL_CENTER.x,
    ROULETTE_FIXED_LANDING_POSITION.y - ROULETTE_WHEEL_CENTER.y,
  ) + 48;
...
function lerpPoint(start: RoulettePoint, end: RoulettePoint, t: number): RoulettePoint {
  return {
...
function polarPosition(radius: number, angleDeg: number): RoulettePoint {
...
function mechanicalBrake(t: number) {
  const x = clamp(t, 0, 1);
  return 1 - (1 - x) ** 2.6;
}

function wheelProgressCurve(progress: number) {
...
function ballDropOffsetFromPosition(point: RoulettePoint): number {
...
function ballAngularProgress(progress: number): number {
  const t = clamp(progress, 0, 1);
...
function ballSettleBounce(progress: number): number {
...
export function sampleRouletteSpinFrame(
  progress: number,
  startWheel: number,
  endWheel: number,
  ballLaps = 7,
...
export function runRouletteWheelSpin({
  fromWheelRotation,
  ...
  const tick = (now: number) => {
    ...
    const frame = sampleRouletteSpinFrame(
      progress,
      fromWheelRotation,
      finalWheelRotation,
      ballLaps,
  ...
  onFrame(
    sampleRouletteSpinFrame(0, fromWheelRotation, finalWheelRotation, ballLaps),
    snapshot,
  );
  rafId = requestAnimationFrame(tick);
