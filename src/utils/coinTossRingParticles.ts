...
type RingParticleVariant = "mote" | "float" | "drift";

function pickVariant(): RingParticleVariant {
  const roll = Math.random();
  if (roll > 0.5) return "mote";
  if (roll > 0.2) return "float";
  return "drift";
}

function spawnRingParticle(container: HTMLElement, stageSizePx: number, coinSizePx: number) {
...
export function startCoinTossRingAmbient(
  ...
  const tick = () => {
    ...
    const capacity = 128;
    if (container.childElementCount >= capacity) return;
