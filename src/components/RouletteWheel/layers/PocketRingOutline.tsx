...
import {
  ...
  ROULETTE_POCKET_BORDER_COLOR,
  ROULETTE_POCKET_BORDER_WIDTH,
...
function dividerLine(centerX: number, centerY: number, angle: number, innerRadius: number, outerRadius: number) {
  return {
...
export function PocketRingOutline() {
  ...
  return (
    <g className="joker-roulette-wheel__pocket-ring-outline" aria-hidden="true">
      <circle
        cx={cx}
        cy={cy}
        r={ROULETTE_POCKET_OUTER_RADIUS}
        fill="none"
        stroke={ROULETTE_POCKET_BORDER_COLOR}
        strokeWidth={ROULETTE_POCKET_BORDER_WIDTH}
        vectorEffect="non-scaling-stroke"
      />
      <circle
        cx={cx}
        cy={cy}
        r={ROULETTE_POCKET_INNER_RADIUS}
        fill="none"
        stroke={RW.pocketFretShadow}
        strokeWidth="2"
        opacity="0.35"
        vectorEffect="non-scaling-stroke"
      />

      {dividers.map(({ key, line, wellLine }) => (
        <g key={`divider-${key}`}>
          <line
            x1={wellLine.inner.x}
            y1={wellLine.inner.y}
