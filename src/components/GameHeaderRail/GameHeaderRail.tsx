...
import type { GameHeaderRailProps } from "./GameHeaderRail.types.js";
...
function HeaderIcon({ icon, className = "" }: { icon?: string; className?: string }) {
  ...
  return <span className={cx("nav-inline-icon-host", className)} dangerouslySetInnerHTML={{ __html: svg }} aria-hidden="true" />;
}
...
export function GameHeaderRail({ game = navigationItemRegistry.crash, gameIcon, rightLabel = "Fair Play", rightIcon = "fair-play", className = "" }: GameHeaderRailProps) {
  ...
  return (
    <div className={cx(styles.root, "joker-game-header-rail", className)} aria-label="Game header">
      <div className={cx(styles.group, "joker-game-header-group")}>
        <span className={cx(styles.info, "joker-game-header-info")} aria-hidden="true">
          <InfoIcon />
        </span>
        <span className={cx(styles.title, "joker-game-header-title")}>
          <HeaderIcon icon={resolvedGameIcon} className={cx(styles.gameIcon, "joker-game-header-game-icon")} />
          <span>{gameLabel}</span>
        </span>
      </div>
      <div className={cx(styles.group, styles.fairPlay, "joker-game-header-group joker-game-header-fair-play")}>
        <HeaderIcon icon={rightIcon} className={cx(styles.fairPlayIcon, "joker-game-header-fair-play-icon")} />
