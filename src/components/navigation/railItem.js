const RAIL_ICONS = {
  home: "/assets/IconHome.svg?v=sessions-rail-v1",
  calendar: "/assets/IconCalendar.svg?v=sessions-rail-v1",
  clients: "/assets/IconClients.svg?v=sessions-rail-v1",
  team: "/assets/IconTeam.svg?v=sessions-rail-v1",
  settings: "/assets/IconSettings.svg?v=sessions-rail-v1",
};

export const RAIL_ITEM_ICONS = ["home", "calendar", "clients", "team", "settings"];

const CALENDAR_ICON = `<svg width="24" height="24" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <circle class="sessions-rail-item__badge" cx="16.6667" cy="3.74996" r="3.33333"/>
</svg>`;
...
export function renderSessionsRailItem({
  ...
  const iconMarkup =
    icon === "calendar"
      ? `<span class="sessions-rail-item__icon sessions-rail-item__icon--svg" aria-hidden="true">${CALENDAR_ICON}</span>`
      : `<span class="sessions-rail-item__icon" style="--sessions-rail-icon: url('${iconSrc}')" aria-hidden="true"></span>`;
...
export function renderSessionsLeftRail({
  selected = "home",
  items = RAIL_ITEM_ICONS,
} = {}) {
  ...
  return `<nav class="sessions-left-rail" aria-label="Primary">${itemMarkup}</nav>`;
}
