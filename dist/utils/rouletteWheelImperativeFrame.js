...
const pointerTickState = new WeakMap();
function getPointerTickState(root) {
...
function resetPointerTick(root) {
    const layer = root.querySelector(".joker-roulette-wheel__pointer-layer");
    layer?.removeAttribute("transform");
    pointerTickState.delete(root);
}
function updatePointerTick(root, wheelRotation) {
    const layer = root.querySelector(".joker-roulette-wheel__pointer-layer");
    if (!layer) {
        return;
    }
    ...
    layer.setAttribute("transform", `rotate(${state.kick.toFixed(3)}, ${state.pivot.x}, ${state.pivot.y})`);
}
/** Apply a spin frame directly to wheel DOM — avoids React re-renders during animation. */
export function applyRouletteWheelSpinFrame(root, frame) {
    const spinner = root.querySelector(".joker-roulette-wheel__wheel-spinner");
    spinner?.style.setProperty("--roulette-wheel-rotation", `${frame.wheelRotation}deg`);
    const ball = root.querySelector(".joker-roulette-wheel__ball");
    if (ball) {
        ...
        ball.style.setProperty("--roulette-ball-bounce-lift", `calc(var(--roulette-wheel-size) * ${frame.ballBounceLift ?? 0} / 1116)`);
    }
    updatePointerTick(root, frame.wheelRotation);
}
export function resetRouletteWheelImperativeFrame(root) {
