...
import { COIN_TOSS_DURATION_MS, computeTossEndRotation, createTossTiltZ, getCoinTossLaunchDelayMs, normalizeTossRestRotation, runCoinTossAnimation, } from "../../utils/coinTossAnimation.js";
...
function applyTossFrame(motionEl, coinEl, shadowEl, frame) {
    if (motionEl) {
...
export function CoinTossable({ side = "heads", tossPhase = "idle", tossOutcome, onTossEnd, tossDurationMs = COIN_TOSS_DURATION_MS, stageSizePx, tapHint, tapHintVisible = true, soundEnabled = true, className, style, ...props }) {
    ...
    const coinSizePx = typeof styleVars?.["--coin-size"] === "string"
        ? Number.parseFloat(String(styleVars["--coin-size"]).replace("px", "")) || 256
        : 256;
    const resolvedStageSizePx = typeof styleVars?.["--coin-toss-stage-size"] === "string"
    ...
    return (_jsxs("div", { className: "joker-coin-toss__arena", style: arenaStyle, children: [_jsx(CoinTossRings, { size: resolvedStageSizePx, coinSizePx: coinSizePx }), _jsxs("div", { className: cx("joker-coin-toss__playfield", isTossing && "is-tossing"), children: [tapHint != null ? (_jsx("p", { className: cx("joker-coin-toss-demo__hint", !tapHintVisible && "is-faded"), children: tapHint })) : null, _jsx("div", { className: "joker-coin-toss__ground", "aria-hidden": "true", children: _jsx("div", { ref: shadowRef, className: "joker-coin-toss__shadow" }) }), _jsx("div", { className: "joker-coin-toss__stage", children: _jsxs("div", { ref: motionRef, className: cx("joker-coin-toss__motion", isTossing && "is-tossing"), children: [_jsx("div", { className: "joker-coin-toss__trail", "aria-hidden": "true" }), _jsx("div", { className: "joker-coin-toss__glow", "aria-hidden": "true" }), _jsx("div", { ref: particlesRef, className: "joker-coin-toss__particles", "aria-hidden": "true" }), _jsx("div", { className: "joker-coin-toss__coin-shadow", "aria-hidden": "true" }), _jsx("div", { ...props, ref: coinRef, className: cx("joker-coin", "joker-coin--tossable", isTossing && "is-tossing", className), style: { "--coin-size": `${coinSizePx}px` }, "aria-hidden": props["aria-hidden"] ?? true, children: _jsx(CoinTossFaces, { pauseSheen: pauseSheen }) })] }) })] })] }));
}
